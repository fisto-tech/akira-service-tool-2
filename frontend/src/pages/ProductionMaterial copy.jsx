...
const ReportsModal = ({ row, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-[1px] z-[100] flex items-center justify-center p-[2vw]">
      <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-[55vw] max-h-[85vh] rounded-[0.4vw] shadow-2xl overflow-hidden flex flex-col border border-slate-400">
        ...
        <div className="flex-1 overflow-y-auto p-[1vw] space-y-[0.8vw]">
          {products.map((prod, idx) => (
             <div key={prod._pid} className="border border-slate-400 rounded-[0.3vw]">
                ...
                {expandedIndex === idx && (
                   <div className="p-[1vw] space-y-[1vw] bg-white border-t border-slate-400">
...
const EmpSelect = ({ label, val, employees, onSelect, disabled }) => {
  ...
  return (
    <div className="flex flex-col gap-[0.1vw] relative" ref={ref}>
       ...
       <div onClick={() => !disabled && setOpen(!open)} className={`flex items-center gap-[0.4vw] bg-white border rounded-[0.25vw] px-[0.6vw] py-[0.35vw] transition-all ${!disabled ? "cursor-pointer border-slate-400 hover:border-black" : "border-slate-400 bg-slate-50"}`}>
       ...
       {open && (
         <div className="absolute top-full left-0 w-full mt-[0.1vw] bg-white border border-slate-400 shadow-xl rounded-[0.3vw] max-h-[12vw] overflow-y-auto z-[200] py-[0.1vw]">
            {employees.map(u => (
...
const EmpSelectCell = ({ val, employees, onSelect }) => {
  ...
  return (
    <div className="relative" ref={ref}>
       <div onClick={() => setOpen(!open)} className={`flex items-center gap-[0.3vw] bg-white border rounded-[0.2vw] px-[0.4vw] py-[0.2vw] transition-all cursor-pointer hover:border-black ${val ? "border-slate-400" : "border-dashed border-slate-400"}`}>
       ...
       {open && (
         <div className="absolute top-full left-0 w-[12vw] mt-[0.1vw] bg-white border border-slate-400 shadow-2xl rounded-[0.3vw] max-h-[10vw] overflow-y-auto z-[200] py-[0.1vw]">
            {employees.map(u => (
  ...
  return (
    <div className="flex items-center gap-[0.4vw]">
      <div className="relative" ref={ref}>
        ...
        <AnimatePresence>
           {open && (
             <motion.div initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="absolute top-full right-0 mt-[0.3vw] bg-white border border-slate-400 shadow-2xl rounded-[0.4vw] z-[100] w-[16vw] p-[0.8vw]">
                ...
                <div className="grid grid-cols-2 gap-[0.4vw] mb-[0.8vw]">
                   {FINAL_STATUS_OPTIONS.map(opt => (
                     <button key={opt} onClick={() => setStatus(opt)} className={`py-[0.35vw] rounded-[0.25vw] border text-[0.65vw] font-bold transition-all ${status === opt ? FINAL_STATUS_COLORS[opt] + " ring-1 ring-offset-1 ring-slate-400" : "bg-slate-50 text-slate-600 border-slate-400 hover:bg-slate-100"}`}>
                        {opt}
                     </button>
                   ))}
                </div>
                <div className="mb-[0.6vw]">
                   ...
                   <input type="date" value={statusDate} onChange={e => setStatusDate(e.target.value)} className="w-full border border-slate-400 rounded-[0.2vw] px-[0.4vw] py-[0.35vw] text-[0.72vw] outline-none focus:border-black text-black font-medium" />
                </div>
                <textarea rows={2} value={remarks} onChange={e => setRemarks(e.target.value)} placeholder="Add remarks..." className="w-full border border-slate-400 rounded-[0.2vw] p-[0.4vw] text-[0.72vw] outline-none focus:border-black resize-none mb-[0.8vw] text-black font-medium" />
                <div className="flex justify-end gap-[0.4vw]">
                   <button onClick={() => setOpen(false)} className="px-[0.8vw] py-[0.3vw] text-[0.65vw] font-bold border border-slate-400 rounded-[0.25vw] hover:bg-slate-50 uppercase">Cancel</button>
      ...
      <div className="relative" ref={historyRef}>
         ...
         <AnimatePresence>
            {showHistory && (
              <motion.div initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="absolute top-full right-0 mt-[0.3vw] bg-white border border-slate-400 shadow-2xl rounded-[0.4vw] z-[100] w-[18vw] p-[0.8vw] overflow-hidden">
                 <div className="flex items-center justify-between mb-[0.8vw] pb-[0.4vw] border-b border-slate-400">
                 ...
                 <div className="max-h-[12vw] overflow-y-auto space-y-[0.6vw] pr-[0.2vw]">
                    {(prod.finalStatusHistory || []).length > 0 ? [...prod.finalStatusHistory].reverse().map((h, i) => (
                      <div key={i} className="bg-slate-50 border border-slate-400 rounded-[0.3vw] p-[0.5vw]">
...
const ProductRow = ({ prod, idx, employees, onUpdate, onRemove, isReadOnly }) => {
  ...
  return (
    <div className="border border-slate-400 rounded-[0.3vw] bg-white shadow-sm mb-[0.8vw]">
      <div className="flex items-center gap-[0.8vw] px-[0.8vw] py-[0.4vw] bg-slate-50 border-b border-slate-400 cursor-pointer" onClick={() => setExpanded(!expanded)}>
              ...
              <div className="p-[0.8vw] space-y-[0.8vw]">
                 ...
                 <div className="grid grid-cols-2 gap-[0.8vw]">
                    <div className="flex flex-col gap-[0.1vw]">
                       ...
                       <input type="number" value={prod.quantity} onChange={e => sp("quantity", e.target.value)} disabled={isReadOnly} className="border border-slate-400 rounded-[0.2vw] py-[0.4vw] px-[0.6vw] text-[0.75vw] text-black font-medium outline-none focus:border-black" />
                    </div>
                    <div className="flex flex-col gap-[0.1vw]">
                       <label className="text-[0.65vw] font-bold text-black uppercase">Batch no / Serial number (Mandatory)</label>
                       <input value={prod.identification} onChange={e => sp("identification", e.target.value)} disabled={isReadOnly} placeholder="Enter Batch/SN..." className="border border-slate-400 rounded-[0.2vw] py-[0.4vw] px-[0.6vw] text-[0.75vw] text-black font-medium outline-none focus:border-black" />
                    </div>
                 </div>
...
const ReadCell = ({ label, val }) => (
  <div className="flex flex-col gap-[0.1vw]">
    ...
    <div className="bg-slate-50 border border-slate-400 rounded-[0.2vw] py-[0.35vw] px-[0.5vw] text-[0.72vw] font-medium text-slate-800 truncate">{val || "—"}</div>
  </div>
);
...
const JOSearchModal = ({ onSelect, onClose, customerDb }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/40 p-[2vw]">
      <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[0.4vw] shadow-2xl w-[35vw] max-h-[65vh] flex flex-col border border-slate-400 overflow-hidden">
        ...
        <div className="p-[0.8vw] border-b border-slate-400 relative bg-slate-50">
           <Search className="absolute left-[1.4vw] top-1/2 -translate-y-1/2 w-[0.9vw] h-[0.9vw] text-slate-400" />
           <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Type product name or Job Order code..." className="w-full pl-[2.2vw] pr-[0.8vw] py-[0.5vw] border border-slate-400 rounded-[0.25vw] text-[0.78vw] text-black outline-none focus:border-black font-medium" />
        </div>
...
const InwardForm = ({ initialData, customerDb, employees, onSave, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden">
      <div className="px-[1.2vw] py-[0.8vw] flex justify-between items-center border-b border-slate-400 bg-white z-[20]">
         ...
         <div className="text-[0.65vw] font-bold font-mono text-slate-400 border border-slate-400 px-[0.65vw] py-[0.15vw] rounded uppercase">{base.refNoInternal}</div>
      </div>

      <div className="flex-1 overflow-y-auto p-[1.5vw]">
         <div className="max-w-[75vw] mx-auto space-y-[1.2vw]">
            <section className="bg-white p-[1.2vw] rounded-[0.4vw] shadow-sm border border-slate-400">
               ...
               <div className="grid grid-cols-3 gap-[1.5vw]">
                  <div className="flex flex-col gap-[0.2vw]">
                     ...
                     <input type="date" value={base.date} onChange={e => setBase({...base, date: e.target.value})} disabled={isReadOnly} className="border border-slate-400 rounded-[0.25vw] py-[0.45vw] px-[0.6vw] text-[0.78vw] text-black font-medium outline-none focus:border-black" />
                  </div>
                  <div className="flex flex-col gap-[0.2vw]">
                     <label className="text-[0.65vw] font-bold text-black uppercase">Stage</label>
                     <select value={base.stage} onChange={e => setBase({...base, stage: e.target.value})} disabled={isReadOnly} className="border border-slate-400 rounded-[0.25vw] py-[0.45vw] px-[0.6vw] text-[0.78vw] text-black font-medium focus:border-black outline-none bg-white">
...
export default function ProductionMaterial() {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white p-[1.2vw] gap-[0.8vw]">
       <div className="flex justify-between items-end border-b border-slate-400 pb-[0.6vw]">
       ...
       <div className="flex items-center gap-[1vw] bg-slate-50 p-[0.6vw] rounded-[0.3vw] border border-slate-400">
          <div className="flex-1 relative">
             <Search className="absolute left-[0.8vw] top-1/2 -translate-y-1/2 w-[0.9vw] h-[0.9vw] text-slate-400" />
             <input value={q} onChange={e => setQ(e.target.value)} placeholder="Filter by Ref No, Product Name or Customer..." className="w-full pl-[2.2vw] pr-[0.8vw] py-[0.5vw] bg-white border border-slate-400 rounded-[0.25vw] text-[0.75vw] font-medium text-black outline-none focus:border-black" />
          </div>
       </div>

       <div className="border border-slate-400 rounded-[0.3vw] flex-1 overflow-y-auto overflow-x-auto">
          <table className="w-full text-left border-collapse text-[0.72vw]">
             <thead>
                <tr className="bg-slate-50 border-b border-slate-400 text-[0.68vw] text-black font-bold uppercase">
                   <th rowSpan={2} className="px-[1vw] py-[0.8vw] border-r border-slate-400">Date / Ref No</th>
                   <th rowSpan={2} className="px-[1vw] py-[0.8vw] border-r border-slate-400">Stage</th>
                   <th colSpan={6} className="px-[1vw] py-[0.5vw] border-r border-slate-400 text-center bg-slate-100/50">Production Unit Details</th>
                   <th rowSpan={2} className="px-[1vw] py-[0.8vw] border-r border-slate-400">Status</th>
                   <th rowSpan={2} className="px-[1vw] py-[0.8vw] border-r border-slate-400">Remarks</th>
                   <th rowSpan={2} className="px-[1vw] py-[0.8vw] border-r border-slate-400 text-center">Control</th>
                </tr>
                <tr className="bg-slate-50 border-b border-slate-400 text-[0.6vw] text-slate-500 font-bold uppercase">
                   <th className="px-[0.8vw] py-[0.4vw] border-r border-slate-400">Product / JO No</th>
                   <th className="px-[0.8vw] py-[0.4vw] border-r border-slate-400">Qty / SN</th>
                   <th className="px-[0.8vw] py-[0.4vw] border-r border-slate-400">Customer</th>
                   <th className="px-[0.8vw] py-[0.4vw] border-r border-slate-400">Assembled</th>
                   <th className="px-[0.8vw] py-[0.4vw] border-r border-slate-400">Tested</th>
                   <th className="px-[0.8vw] py-[0.4vw] border-r border-slate-400">FI By</th>
                </tr>
             </thead>
             <tbody className="divide-y divide-slate-100">
                {filteredData.flatMap(row => {
                   ...
                   return products.map((prod, pi) => (
                      <tr key={`${row.id}-${pi}`} className={`hover:bg-slate-50/50 transition-colors ${pi === 0 ? "border-t border-slate-400" : "border-t border-slate-400 border-dashed"}`}>
                         {pi === 0 && (
                            <>
                               <td rowSpan={span} className="px-[1vw] py-[0.8vw] border-r border-slate-400 align-top">
                               ...
                               <td rowSpan={span} className="px-[1vw] py-[0.8vw] border-r border-slate-400 align-top">
                                  <span className="text-slate-700 font-bold uppercase border border-slate-400 bg-white px-[0.4vw] py-[0.1vw] rounded text-[0.65vw]">{row.stage}</span>
                               </td>
                            </>
                         )}
                         <td className="px-[0.8vw] py-[0.6vw] border-r border-slate-400">
                         ...
                         <td className="px-[0.8vw] py-[0.6vw] border-r border-slate-400 font-medium whitespace-nowrap">
                         ...
                         <td className="px-[0.8vw] py-[0.6vw] border-r border-slate-400 text-slate-600 font-medium text-[0.7vw] uppercase max-w-[10vw] truncate" title={prod.customerName}>
                           {prod.customerName}
                         </td>
                         <td className="px-[0.8vw] py-[0.6vw] border-r border-slate-400">
                         ...
                         <td className="px-[1vw] py-[0.8vw] border-r border-slate-400">
                         ...
                         {pi === 0 && (
                           <td rowSpan={span} className="px-[1vw] py-[0.8vw] flex flex-wrap justify-center gap-[0.4vw] items-center border-l border-slate-400">
                              <button onClick={() => setShRep(row)} title="View Reports" className="p-[0.4vw] border border-slate-400 text-slate-400 rounded-[0.3vw] font-bold hover:bg-slate-900 hover:text-white transition-all cursor-pointer"><Eye className="w-[1vw] h-[1vw]" /></button>
